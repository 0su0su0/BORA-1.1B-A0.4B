...
- **Per-layer bias update speed (γ)**: 레이어별 라우팅 편향 조정 속도
  - DeepSeek-V3는 모든 레이어에 γ=0.001 단일 값 사용
  - BORA는 레이어 깊이에 따라 편향 업데이트 속도를 차등 적용
  - **얕은 레이어**: 낮은 γ (0.0001) - expert 라우팅 진동 방지
  - **깊은 레이어**: 높은 γ (0.0005) - 적극적 로드 밸런싱
